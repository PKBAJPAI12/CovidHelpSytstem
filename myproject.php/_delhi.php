...
<input id="input" class="form-control mr-sm-2" type="search" placeholder="Search Hospitals" aria-label="Search" onkeyup="searchstatehelp()">
        ...
        <table class="table border border-dark my-5" id="table1">
            <thead>
                <tr>
                    <th class="table-info" scope="col">Hospital Name</th>
                    <th class="table-info" scope="col">Last Updated</th>
                    <th class="table-info" scope="col">Total Beds</th>
                    <th class="table-info" scope="col">Vacant Beds</th>
                    <th class="table-info" scope="col">Oxygen Left [Hours]</th>
                </tr>
            </thead>
            <tbody>
            <?php 
  $sql="SELECT * FROM `covid_csv___sheet1`";
  $result=mysqli_query($conn,$sql); 
  while($row = mysqli_fetch_assoc($result)){
      if($row['COL 4']<50)
echo "<tr>        
                    <td class='table-danger'>" .$row['COL 1']. "</td>
                    <td class='table-danger'>" .$row['COL 2']. "</td>
                    <td class='table-danger'>" .$row['COL 3']. "</td>
                    <td class='table-danger'>" .$row['COL 4']. "</td>
                    <td class='table-danger'>" .$row['COL 5']. "</td>
                </tr>";
                if($row['COL 4']>80)
                echo "<tr>
                    <td class='table-success'>" .$row['COL 1']. "</td>
                    <td class='table-success'>" .$row['COL 2']. "</td>
                    <td class='table-success'>" .$row['COL 3']. "</td>
                    <td class='table-success'>" .$row['COL 4']. "</td>
                    <td class='table-success'>" .$row['COL 5']. "</td>
                </tr>";
                if($row['COL 4']>=50 && $row['COL 4']<=80)
                echo "<tr>
                    <td class='table-warning'>" .$row['COL 1']. "</td>
                    <td class='table-warning'>" .$row['COL 2']. "</td>
                    <td class='table-warning'>" .$row['COL 3']. "</td>
                    <td class='table-warning'>" .$row['COL 4']. "</td>
                    <td class='table-warning'>" .$row['COL 5']. "</td>
                </tr>";
  }
                ?>          
            </tbody>
